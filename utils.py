...
def gen_adj_mat_tensor(data, parameter, metric="cosine"):
    ...
    adj = to_sparse(adj)
    #for write train_adj
    # new_trian_adj=copy.deepcopy(adj)
    # new_trian_adj=new_trian_adj.to_dense()
    # new_trian_adj[new_trian_adj==0]=255
    # new_trian_adj[new_trian_adj<255]=0
    # cv2.imwrite('train_adj.jpg',new_trian_adj.cpu().numpy())#for write train_adj
    return adj


def gen_test_adj_mat_tensor(data, trte_idx, parameter, adj_parameter, metric="cosine"):
    assert metric == "cosine", "Only cosine distance implemented"
    ...
    #GCN forte2te
    dist_te2te = cosine_distance_torch(data[trte_idx["te"]], data[trte_idx["te"]])
    g_te2te = graph_from_dist_tensor(dist_te2te, parameter/1, self_dist=False)#
    if metric == "cosine":
        adj[num_tr:,num_tr:] = 1-dist_te2te
    else:
        raise NotImplementedError
    adj[num_tr:,num_tr:] = adj[num_tr:,num_tr:]*g_te2te
    new_adj=copy.deepcopy(adj)
    new_adj[new_adj==0]=255
    new_adj[new_adj<255]=0
    #cv2.imwrite('p_test_1.jpg',new_adj.cpu().numpy())
